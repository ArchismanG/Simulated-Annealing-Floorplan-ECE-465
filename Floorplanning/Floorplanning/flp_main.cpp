...
int main(int argc, char* argv[])
{
    ...
    int moves_per_temp = 20000; // Simulated Annealing Parameters defaults
    block_area = 0.0; // Default
    ...
    std::vector<Chiplet> myChiplets = parseChiplets(blockfile, indexmap); // Modules Parsed
    // sortChiplets(myChiplets, 0, myChiplets.size() - 1); // Sort Chiplets in descending order height

    for (int i = 0;i < myChiplets.size();i++)
        block_area += (myChiplets[i].x_dim * myChiplets[i].y_dim); // Calculate Area occupied by the blocks
    ...
    IntialFloorplan.IndexMap = indexmap;
    ...
    // numCores = 1; For Testing
    std::vector<std::thread> threads;
    std::vector<FloorplanState> threadResults(numCores);
    LIVE_TEMPS.assign(numCores, start_temp);    // Fills it with starting temperature
    ...
    calculateHPWL(IntialFloorplan.chiplets, myNets, IntialFloorplan.IndexMap, IntialFloorplan.hpwl);
    ...
    std::cout << "The parsed outline for the dataset is " << x_outline << " X " << y_outline << "\n"; // Display the Bounding Box value
    // displayFloorplan(threadResults[BestResult].chiplets, threadResults[BestResult].IndexMap);
    ...
    // displayRelationMatrix(threadResults[BestResult].relationMatrix, threadResults[BestResult].id_relation);
    // std::cin.get();
    return 0;
}