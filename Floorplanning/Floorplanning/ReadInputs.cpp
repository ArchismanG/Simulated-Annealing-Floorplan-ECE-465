...
std::vector<Net> parseNets(const std::string& filename, const std::vector<Chiplet>& chiplets) 
{
    ...
    while (std::getline(file, line)) {
        ...
        if (token == "NetDegree:") {
            ...
            if (isParsingNet && (currentNet.chipletIDs.size() > 1)) // Dont want to push nets with just 1 ID 
            {
    ...
    // Push the very last net after the loop ends!
    if (isParsingNet && (currentNet.chipletIDs.size() > 1))
    {
        nets.push_back(currentNet);
    }
